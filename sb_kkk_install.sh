...
# 【优化点1】：强制临时文件夹在物理磁盘下，防止 tmpfs 爆内存
tmpdir=/root/.sb_tmp_$RANDOM
mkdir -p $tmpdir
...
check_status() {
    ...
    if [[ $is_wget ]]; then
        ...
        [[ ! $is_fail ]] && {
            is_wget=1
            # 【优化点2】：取消所有任务的并发符 "&" 和 "wait"，强制依次单线程执行防止OOM
...
main() {
    ...
    # 【优化点3】：针对 64MB 内存注入环境变量限制，强制进行积极的垃圾回收
    msg warn "检测到您可能处于极小内存环境，正在注入极限内存控制配置(GOGC=20)..."
    if [[ $(type -P systemctl) ]]; then
        mkdir -p /etc/systemd/system/${is_core}.service.d
        cat <<EOF > /etc/systemd/system/${is_core}.service.d/lowram.conf
[Service]
Environment="GOGC=20"
...
EOF
        ...
        mkdir -p /etc/conf.d/
        echo 'export GOGC=20' >> /etc/conf.d/${is_core}
        echo 'export GOMEMLIMIT=20MiB' >> /etc/conf.d/${is_core}
    fi
